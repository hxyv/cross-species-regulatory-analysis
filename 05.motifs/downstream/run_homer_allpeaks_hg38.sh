...
# HOMER motif analysis on ALL human peaks — hg38, default background
#
# Purpose:
#   Run HOMER findMotifsGenome.pl on the complete (unfiltered) set of human
...
#   sets to get a general motif profile of all open chromatin in human
...
#   2. Auto-generates background sequences from hg38 (GC/repeat-matched)
...
#
# 2 analyses:
#   1) humanenhancers.sorted.bed → all human enhancer peaks (92,105 regions)
#   2) humanpromoters.sorted.bed → all human promoter peaks (25,108 regions)
#
# Prerequisite:
...
# internal helpers (bed2pos.pl, homer2,, etc.).
...
GENOME="hg38"    # Human reference genome (must be pre-installed in HOMER)
...
# Full (unfiltered) human enhancer and promoter peak sets in hg38 coordinates.
# These are the complete ATAC-seq peak sets — not subsetted by species-specificity
# or conservation status.
check_file "humanenhancers.sorted.bed"    # 92,105 peaks
check_file "humanpromoters.sorted.bed"    # 25,108 peaks
...
# All human enhancers: general motif profile of human adrenal enhancers
# → Answers: what TF motifs are enriched across ALL enhancers vs random genome?
run_cmd "humanenhancers.sorted.bed"  "out_allHumanEnh_hg38_defaultBg"  "1/2"

# All human promoters: general motif profile of human adrenal promoters
# → Answers: what TF motifs are enriched across ALL promoters vs random genome?
run_cmd "humanpromoters.sorted.bed"  "out_allHumanProm_hg38_defaultBg" "2/2"

echo "All human all-peaks HOMER analyses completed successfully."
