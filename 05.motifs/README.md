...
<p align="center">
  <img src="assets/task5.png" alt="Cross-species regulatory analysis workflow" width="1000">
</p>
...
2. Use sort_bed_files.sh to get the BED files sorted.
3. `downstream/build_centered_beds.sh` creates 200 bp summit-centered peak windows.
4. `downstream/run_all.sh` runs the HOMER analyses for human and mouse, centered and non-centered windows, and all-peak backgrounds.
5. `downstream/summarize_known_motifs.py` summarizes HOMER `knownResults.txt` files into `downstream/results_summary/`.
